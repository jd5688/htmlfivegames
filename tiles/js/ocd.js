...
var sprite_width 	= 0;
var sprite_height 	= 0;
var orig_canvas_width = 640;
var orig_canvas_height = 960;
var theme;
...
function refreshCanvas() {
   ...
   // canvas.attr("height", $(window).get(0).innerHeight / 2);
    game_width = canvas.attr('width');
    game_height = canvas.attr('height');
};
...
var Drawer = function (ctx) {
	return {
		...
		draw : function (x, y, w, h, sprite_x, sprite_y) {
			...
			rh = (game_height / orig_canvas_height) * h;
			rw = (game_width / orig_canvas_width) * w;

			//ctx.fillStyle = 'rgba(255, 255, 255, 0.5)';
			//ctx.strokeRect(x, y, w, h);
			ctx.drawImage(imgSprite, sprite_x, sprite_y, w, h, this.x, this.y, w, h);
		},
		coord : function () {
			return {
				x : this.x,
...
var Tiles = function (rectangle, context, canvas, footstep) {
	return {
		...
		margin: game_width * .15, // constraint the tiles n% from left and from right of canvas
		/*
		draw : function () {
			var x = 0;
			var y = 0;
		...
		*/
		draw : function () {
			...
			// the top tile
			y = (game_height / 2) - (this.HEIGHT / 2) - this.HEIGHT;
			rectangle.draw(x,y, this.WIDTH, this.HEIGHT, this.TILE_SPRITE_X, this.TILE_SPRITE_Y);
			this.rectangles[this.rectangles.length] = rectangle.coord();
			thing = Thing();
			this.rectangles[this.rectangles.length - 1].thing = thing;
		...
		randX: function () {
			var x = Math.floor(Math.random() * (game_width - (this.margin + this.WIDTH)));
			if (x < this.margin) {
				x = x + this.margin;
			}
			return x;
		...
		move: function() {
			...
			for (var i = 0; i < len; i += 1) {
				...
				this.rectangles[i].x = x;

				// check if tile is already outside canvas
				if (this.rectangles[i].x < this.WIDTH * -1) {
...
function initialize(canvas) {
	$(canvas).mousedown(drawFootstep);
	//canvas.addEventListener("mousedown", drawFootstep, false);
   	theme = new Audio("./sound/theme.mp3");
   	theme.loop = true;
}
...
function drawFootstep(event) {
    ...
    x = (event.clientX - rect.left) / (rect.right - rect.left) * game_width,
	y = (event.clientY - rect.top) / (rect.bottom - rect.top) * game_height
